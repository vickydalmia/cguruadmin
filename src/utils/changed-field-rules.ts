// The changed-field rule tables: per-field validity closures, messages, and
// editor-facing hints for the entity content types and the shared SEO
// component. Enforced by ./changed-field-validation (write time) and
// surfaced by ./changed-field-hints (edit-form descriptions) — one table, so
// what is shown is exactly what is enforced.
...
export const TOP_LEVEL_RULES: Rule[] = [
  topRule(
    'api::store.store',
    'name',
...
export const SEO_RULES: Omit<Rule, 'uid'>[] = [
  {
    path: ['seo', 'metaTitle'],
    valid: maxLength(70),
