...
| Search results correct but latency high under load | indexes fine, caches cold | `/api/search` has a short in-process cache and the gateway layers its own; inspect CMS timing logs and gateway `/healthz` search metrics |
